...
export type DecodedTlvMap = Map<number, Buffer[]>;
...
export function decodeUVarint(
  ...
  for (let i = 0; i < limits.maxVarintBytes; i++) {
    ...
    if ((b & 0x80) === 0) {
      ...
      // Minimal-encoding check: re-encode and compare exact bytes
...
export function encodeTLVStreamCanonical(entries: DecodedTlv[]): Buffer {
  ...
  return Buffer.concat(sorted.map((e) => encodeTLV(e.tag, e.value)));
}
...
export function decodeTLVStream(
  ...
  while (off < stream.length) {
    ...
    out.push({ tag, value: Buffer.from(stream.subarray(off, off + len)) });
    off += len;
  }
...
export function decodeAxisHeaderFromTLVs(
  ...
  const m = new Map(hdrTlvs.map((t) => [t.tag, t.value]));
  return {
    intentId: Number(decodeUVarint(m.get(1)!, 0).value),
    actorKeyId: m.get(2)!,
    capsuleId: m.get(3),
    nonce: m.get(4)!,
    tsMs: m.get(5)!.readBigUInt64BE(0),
    schemaId: Number(decodeUVarint(m.get(6)!, 0).value),
    bodyHash: m.get(7)!,
    traceId: m.get(8),
  };
}
