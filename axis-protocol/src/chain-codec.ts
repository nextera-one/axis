...
import { decodeTLVs, decodeTLVsList, encodeTLVs, type TLV } from './tlv';
...
export function decodeChainEnvelope<TInput = unknown>(
  ...
  return {
    ...
    issuedAtTps: decodeMaybeString(map.get(ChainEnvelopeTag.ISSUED_AT_TPS)),
    expiresAtTps: decodeMaybeString(map.get(ChainEnvelopeTag.EXPIRES_AT_TPS)),
...
>(request: AxisChainRequest<TInput, TCapsule>): Uint8Array {
  const tlvs: TLV[] = [
    {
      type: ChainRequestTag.ENVELOPE,
      value: encodeChainEnvelope(request.envelope),
    },
  ];
...
function decodeMaybeString(value?: Uint8Array): string | undefined {
  const decoded = decodeUtf8(value);
...
function decodeScope(value?: Uint8Array): string | string[] | undefined {
  const items = decodeStringList(value);
